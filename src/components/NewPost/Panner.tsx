...
export class Panner extends React.Component<Props> {
  static defaultProps = {
  ...
  componentDidUpdate(prevProps: Props) {
    ...
    if (
      ...
      (index !== prevProps.index && index !== this.currentIndexValue) ||
    ...
    if (prevProps.length !== length) {
      this.routesLength.setValue(length);
    }
  ...
  private progress = new Value(
    ...
    this.props.index * this.props.layout.width * DIRECTION_RIGHT
  );

  // Initial index of the tabs
  private index = new Value(this.props.index);
  ...
  private lastEnteredIndex = new Value(this.props.index);
  ...
  private indexAtSwipeEnd: Animated.Value<number> = new Value(this.props.index);

  // Mappings to some prop values
  // We use them in animation calculations, so we need live animated nodes
  private routesLength = new Value(this.props.length);
  ...
  private currentIndexValue = this.props.index;
  ...
  private handleEnteredIndexChange = ([value]: readonly number[]) => {
    const index = Math.max(0, Math.min(value, this.props.length - 1));
  ...
  render() {
    ...
    return (
      <>
        <Animated.Code
          exec={Animated.block([
            Animated.set(position, this.position),
            isSwiping ? Animated.set(this.isSwiping, isSwiping) : 0,
            block([
              onChange(
                this.index,
