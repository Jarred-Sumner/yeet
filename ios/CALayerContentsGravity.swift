...
*/
public enum kCALayerContentsGravity: String {
...
struct ContentsGravityTransformation {
  ...
  init(for layer: CALayer, contentSize: CGSize) {
        let scaledContents = CGSize(
            width: CGFloat(contentSize.width) / layer.contentsScale,
            height: CGFloat(contentSize.height) / layer.contentsScale
        )
